...
func (r *ProductRepository) Create(input models.ProductInput) error {
	...
	_, err := r.DB.Exec(context.Background(), query, input.KategoryID, input.Name, input.Description, input.Price, input.ImageURL)
	return err
}
...
func (r *ProductRepository) Update(id int, input models.ProductInput) error {
	...
	_, err := r.DB.Exec(context.Background(), query, input.KategoryID, input.Name, input.Description, input.Price, input.ImageURL, id)
	return err
}
